(ns step-definitions.kaocha-integration
  ...
  (with-open [deps-out (io/writer path)]
    (binding [*out* deps-out]
      (with-print-namespace-maps false
        (clojure.pprint/pprint {:deps {'lambdaisland/kaocha           {:mvn/version "RELEASE"}
                                       'lambdaisland/kaocha-cloverage {:mvn/version "RELEASE"}
                                       'lambdaisland/kaocha-cucumber  {:local/root (project-dir-path)}}})))))
